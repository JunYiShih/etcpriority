...
if all_args.cuda and torch.cuda.is_available():
    ...
    torch.set_num_threads(all_args.n_training_threads)
n_walkers = 6
env = multiwalker_com.parallel_env(n_walkers=n_walkers, position_noise=0, 
...
frames = []
model_dir = os.path.join(os.path.dirname(__file__), 'model_walker_p_6')
# actor1.load_state_dict(torch.load('actor_agent0.pt'))
# actor1.eval()
# actor2.load_state_dict(torch.load('actor_agent1.pt'))
# actor2.eval()
# actor3.load_state_dict(torch.load('actor_agent2.pt'))
# actor3.eval()
...
rnn_state = np.zeros(( n_walkers, recurrent_N, all_args.actor_hidden_size), dtype=np.float32)
...
for _ in range(500):  # Adjust the number of steps if needed
    ...
    while env.agents:
      ...
      image = env.render()
      frames.append(image)
writer = imageio.get_writer('render.mp4', fps=24)
